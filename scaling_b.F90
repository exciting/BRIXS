program scaling_b
  ...
  real(8) :: start, finish, runtime, runtime2, runtime3
  character(1024) :: ichr, fname_output
  ...
  do exponent_=1,5
    ...
    call cpu_time(start)
    call rixs2(exponent_,nblocks_,nk_,blsz_)
    call cpu_time(finish)
    runtime3=finish-start
    write(6,*) exponent_, runtime, runtime2, runtime3
  end do
  close(6)
  call hdf5_finalize()
...
subroutine rixs2(i,nblocks_,nk_,blsz_)
  use mod_hdf5
  use mod_io
  use mod_rixs
  use mod_matmul
  ...
  integer, intent(in) :: nk_
  integer, intent(in) :: blsz_
  ! internal variables
  real(8) :: broad, broad2
  real(8), allocatable :: omega(:), omega2(:)
  real(8) :: pol(3)
